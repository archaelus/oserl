...
%%%   <tr>
%%%     <td valign="top">
%%%       <a href="#transmitter_mc_unavailable-2">transmitter_mc_unavailable/2
%%%       </a>
%%%     </td>
...
%%%   <tr>
%%%     <td valign="top">
%%%       <a href="#transceiver_mc_unavailable-2">transceiver_mc_unavailable/2
%%%       </a>
%%%     </td>
...
%%%     </td>
%%%     <td>Forwards unbind requests (issued by the MC) on the receiver 
%%%       session.
%%%     </td>
%%%   </tr>
%%%   <tr>
...
%%%
%%% <p><tt>PduResp</tt> is the PDU response sent by the MC.  <tt>Pid</tt> is 
%%% the ESME parent id, <tt>Eid</tt> as the ESME process id.</p>
%%%
...
%%% operation, when the MC closes the underlying connection first.  The ESME 
%%% must handle these <i>undesired</i> callbacks appropriately.</p>
%%% 
%%% <p><tt>Pid</tt> is the ESME's parent id, <tt>Eid</tt> as the ESME
%%% process id.</p>
%%%
...
%%% @version 1.0 beta, {10 Jun 2003} {@time}.
%%% @end
-module(gen_esme).
...
%%
%% <p><tt>Setup</tt> is a esme_setup() record with all the ESME setup 
%% parameters.  Use the macro ?DEFAULT_ESME_SETUP to set the default values.
%% </p>
%%
%% <p>Refer to <b>gen_esme.hrl</b> for details on the esme_setup()
%% record definition.</p>
%%
...
init([Pid, Module, Setup]) ->
    State = #state{parent          = Pid, 
                   ...
                   system_id       = Setup#esme_setup.system_id,
                   password        = Setup#esme_setup.password,
                   addr_ton        = Setup#esme_setup.addr_ton,
                   addr_npi        = Setup#esme_setup.addr_npi,
                   address_range   = Setup#esme_setup.address_range,
                   source_addr_ton = Setup#esme_setup.source_addr_ton,
                   source_addr_npi = Setup#esme_setup.source_addr_npi,
                   source_addr     = Setup#esme_setup.source_addr,
                   service_type    = Setup#esme_setup.service_type,
                   system_type     = Setup#esme_setup.system_type,
                   session_setup   = ?GET_ESME_SESSION_SETUP(Setup)},
